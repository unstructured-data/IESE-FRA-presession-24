# IESE-FRA-presession-24
Pre-session materials for the [Large Language Models and Machine Learning for Unstructured Data Seminar]([url](https://www.iese.edu/faculty-research/large-language-models-machine-learning/)), organized by IESE and the Fundación Ramón Areces.

## Main repository for the seminar

[GitHub link](https://github.com/unstructured-data/IESE-FRA-seminar-24/tree/main)

## Python resources:

- [freeCodeCamp Python Tutorial](https://www.youtube.com/watch?v=eWRfhZUzrAc)
- [W3Schools Introduction to Python](https://www.w3schools.com/python/python_intro.asp)

## Pandas resources:

- [Pandas official documentation](https://pandas.pydata.org/docs/)
- [W3Schools Pandas Introduction](https://www.w3schools.com/python/pandas/pandas_intro.asp)

## GitHub resources:

- [GitHub fork](https://docs.github.com/en/get-started/quickstart/fork-a-repo)
